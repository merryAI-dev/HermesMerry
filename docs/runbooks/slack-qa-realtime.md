...
- GitHub 이슈화: `--send` 운영 시 Hermes 1차 진단을 GitHub issue로 생성
- Slack 응답: Hermes 1차 진단은 QA 접수 메시지 스레드 댓글, GitHub issue 링크와 보람 멘션은 채널 일반 메시지로 발송
...
Slack 채널에는 아래 형식으로 일반 메시지를 남깁니다. Hermes 진단 본문은 QA 접수 메시지의 스레드 댓글로 남깁니다.
